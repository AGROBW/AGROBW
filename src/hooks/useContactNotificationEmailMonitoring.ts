...
const mapJob = (row: any): ContactNotificationEmailJob => ({
  id: row.id,
  sourceKind: row.source_kind,
  messageId: row.message_id ?? null,
  leadId: row.lead_id ?? null,
  guestContactId: row.guest_contact_id ?? null,
...
const mapLog = (row: any): ContactNotificationEmailDispatchLog => ({
  id: row.id,
...
export const useContactNotificationEmailMonitoring = () => {
  const [summary, setSummary] = useState<ContactNotificationEmailSummary>({
    pending: 0,
    failed: 0,
    skipped: 0,
    sentToday: 0,
  });
  const [jobs, setJobs] = useState<ContactNotificationEmailJob[]>([]);
  const [dispatchLogs, setDispatchLogs] = useState<ContactNotificationEmailDispatchLog[]>([]);
  ...
  const processQueueNow = useCallback(
    async (limit = 25) => {
      const { data, error } = await supabase.functions.invoke('sync-contact-notification-emails', {
        body: { limit },
      });
