...
const mapPaymentRecord = (row: any): PaymentRecord => ({
  ...
  billingModel: row.billing_model ?? row.metadata?.billing_model ?? null,
  ...
  fiscalStatus: row.fiscal_status ?? 'not_requested',
  fiscalLastAttemptAt: row.fiscal_last_attempt_at ?? null,
  fiscalErrorMessage: row.fiscal_error_message ?? null,
