...
export const fetchAdminNotificationItems = async (): Promise<AdminNotificationItem[]> => {
  ...
  try {
    const [
      ...
      supabase.from('announcements').select('created_at').eq('status', 'PENDING').order('created_at', { ascending: false }).limit(1).maybeSingle(),
    ...
    if (pendingAnnouncementsCount > 0) {
      items.push({
        ...
        content: 'Existem novos anuncios pendentes de aprovacao na fila de moderacao.',
