...
import { useRadarMatchEmailMonitoring } from '../../src/hooks/useRadarMatchEmailMonitoring';
...
const RadarMatchEmailManagement: React.FC = () => {
  ...
  const handleProcessNow = async () => {
    ...
    if (result.error) {
      toast.error('Nao foi possivel processar a fila do Radar agora.', {
    ...
    toast.success('Fila do Radar processada com sucesso.', {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-[30px] border border-slate-200 bg-white px-6 py-6 shadow-[0_24px_80px_-48px_rgba(15,23,42,0.3)] lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <h2 className="mt-4 text-2xl font-black text-slate-950">Monitoramento dos envios do Radar</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
            Acompanhe a fila de e-mails gerada pelos matches do Radar, veja falhas e force um processamento manual quando precisar.
          </p>
        </div>
      ...
      {error ? (
        <div className="rounded-[24px] border border-rose-200 bg-rose-50 px-5 py-4 text-sm font-medium text-rose-700">
          Nao foi possivel carregar o monitoramento de e-mails do Radar: {error}
        </div>
      ) : null}
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <SummaryCard
          ...
          helper="Entregas registradas hoje"
        ...
        <SummaryCard
          ...
          helper="Sem envio por regra ou dado ausente"
      ...
      <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_22px_60px_-42px_rgba(15,23,42,0.26)]">
        <div className="flex flex-col gap-4 border-b border-slate-100 pb-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h3 className="mt-2 text-lg font-black text-slate-950">Ultimos jobs do Radar</h3>
          </div>
              ...
              {isLoading ? (
                ...
                filteredJobs.map((job) => (
                  <tr key={job.id} className="align-top">
                    ...
                    <td className="px-4 py-4">
                      <p className="font-semibold text-slate-900">{job.announcementTitle || 'Anuncio sem titulo'}</p>
                      <p className="mt-1 text-xs text-slate-500">
                        {job.alertName ? `Alerta: ${job.alertName}` : `Anuncio ${job.announcementId.slice(0, 8)}...`}
                      </p>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4 text-sm text-slate-600">{formatDateTime(job.queuedAt)}</td>
                    <td className="px-4 py-4 text-sm text-slate-600">
                      {job.status === 'sent' ? formatDateTime(job.sentAt) : formatDateTime(job.lastAttemptAt)}
                    </td>
                    <td className="px-4 py-4 text-sm text-slate-500">
                      {job.lastError ? (
                        <span className="line-clamp-2">{job.lastError}</span>
                      ) : (
                        <span className="text-slate-400">Sem ocorrencias</span>
                      )}
                    </td>
                  </tr>
                ))
        ...
        <div className="mt-5 space-y-3">
          {isLoading ? (
            ...
            <div className="rounded-[22px] border border-dashed border-slate-200 px-5 py-10 text-center text-sm text-slate-500">
              Ainda nao houve execucao registrada para a fila do Radar.
            </div>
          ) : (
            dispatchLogs.map((log) => (
              <div
                key={log.id}
                className="rounded-[22px] border border-slate-200 bg-slate-50/70 px-5 py-4"
              >
...
export default RadarMatchEmailManagement;
