...
import { usePlanAlertEmailMonitoring } from '../../src/hooks/usePlanAlertEmailMonitoring';
...
const PlanAlertEmailManagement: React.FC = () => {
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job) => {
      ...
      if (kindFilter !== 'all' && job.alertKind !== kindFilter) return false;
      return true;
    });
  ...
  const handleProcessNow = async () => {
    ...
    if (result.error) {
      toast.error('Nao foi possivel processar a fila de alertas agora.', {
    ...
    toast.success('Fila de alertas processada com sucesso.', {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 rounded-[30px] border border-slate-200 bg-white px-6 py-6 shadow-[0_24px_80px_-48px_rgba(15,23,42,0.3)] lg:flex-row lg:items-center lg:justify-between">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-black uppercase tracking-[0.26em] text-emerald-700">
            ...
            Conversao e Renovacao por E-mail
          </div>
          <h2 className="mt-4 text-2xl font-black text-slate-950">Monitoramento dos alertas comerciais</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
            Acompanhe os envios por e-mail das notificacoes de conversao e renovacao, veja falhas e processe a fila manualmente quando precisar.
          </p>
        </div>
      ...
      {error ? (
        <div className="rounded-[24px] border border-rose-200 bg-rose-50 px-5 py-4 text-sm font-medium text-rose-700">
          Nao foi possivel carregar o monitoramento dos alertas: {error}
        </div>
      ) : null}
      ...
      <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_22px_60px_-42px_rgba(15,23,42,0.26)]">
        <div className="flex flex-col gap-4 border-b border-slate-100 pb-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h3 className="mt-2 text-lg font-black text-slate-950">Ultimos jobs de conversao e renovacao</h3>
          </div>
              ...
              {isLoading ? (
                ...
                filteredJobs.map((job) => (
                  <tr key={job.id} className="align-top">
                    ...
                    <td className="px-4 py-4">
                      <span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold ${kindMeta[job.alertKind].className}`}>
                        {kindMeta[job.alertKind].label}
                      </span>
                    </td>
                    <td className="px-4 py-4">
                      <p className="font-semibold text-slate-900">{job.notificationTitle}</p>
                      <p className="mt-1 line-clamp-2 text-xs text-slate-500">{job.notificationContent}</p>
                    </td>
                    <td className="px-4 py-4">
        ...
        <div className="mt-5 space-y-3">
          {isLoading ? (
            ...
            <div className="rounded-[22px] border border-dashed border-slate-200 px-5 py-10 text-center text-sm text-slate-500">
              Ainda nao houve execucao registrada para a fila de alertas.
            </div>
          ) : (
...
export default PlanAlertEmailManagement;
