...
type PlanAlertEmailJobRow = {
  id: string;
  recipient_email: string | null;
  recipient_name: string | null;
  alert_kind: 'conversion' | 'renewal';
...
const getPlanAlertTemplate = (params: {
  appUrl: string;
  siteName: string;
  userName: string;
  title: string;
  ...
  const ctaLabel = params.alertKind === 'conversion' ? 'Ver oportunidade' : 'Ver meu plano';
  const badgeLabel = params.alertKind === 'conversion' ? 'Conversao inteligente' : 'Renovacao inteligente';
  const footerLine =
    params.alertKind === 'conversion'
      ? `Esse aviso foi enviado porque seu anuncio demonstrou potencial comercial na ${params.siteName}.`
      : `Esse aviso foi enviado para ajudar voce a manter o plano ativo na ${params.siteName}.`;
      ...
      <body style="margin:0;padding:24px;background:#f8fafc;font-family:Arial,sans-serif;color:#0f172a;">
        <div style="max-width:620px;margin:0 auto;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e2e8f0;">
          <div style="padding:28px 32px;background:#0f172a;color:#ffffff;">
            ...
            <h1 style="margin:0;font-size:24px;line-height:1.2;">${params.title}</h1>
          </div>
...
serve(async (req) => {
  ...
  try {
    ...
    const cronSecret = Deno.env.get('PLAN_ALERT_EMAILS_CRON_SECRET');
    ...
    for (const job of (jobRows || []) as PlanAlertEmailJobRow[]) {
      ...
      const email = getPlanAlertTemplate({
        ...
        title: claimedJob.notification_title,
        content: claimedJob.notification_content,
        link: claimedJob.link,
        alertKind: claimedJob.alert_kind,
      });
      ...
      if (result.success) {
        sentCount += 1;
        await supabaseAdmin.from('plan_alert_email_jobs')
    ...
    return jsonResponse({ success: false, error: 'Erro interno ao processar alertas de plano' }, 500);
  }
});
