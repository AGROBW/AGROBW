...
-- - Contatos recebidos durante a vigencia de um plano pago permanecem acessiveis
-- - Novos contatos recebidos fora da vigencia ficam bloqueados na aba Recebidas
-- - O comprador continua podendo enviar mensagens normalmente
-- - O vendedor so fica bloqueado para responder contatos recebidos sem acesso pago
-- ============================================================================
...
create or replace function public.seller_has_active_paid_contact_access(
  p_seller_id uuid,
...
begin
  select exists (
    ...
    where us.user_id = p_seller_id
      ...
      and coalesce(p.is_default_signup_plan, false) = false
      and coalesce(p.is_downgrade_plan, false) = false
      and (
        coalesce(p.monthly_price, 0) > 0
        or coalesce(p.yearly_price, 0) > 0
        or us.source = 'promotion'
        or us.promotion_code_id is not null
      )
  )
  into v_has_access;
...
begin
  ...
  new.received_with_active_access := public.seller_has_active_paid_contact_access(
    new.seller_id,
    new.created_at
  );
...
set received_with_active_access = public.seller_has_active_paid_contact_access(
      l.seller_id,
...
begin
  ...
  set contact_expires_at = case
    ...
    when public.seller_has_active_paid_contact_access(l.seller_id, now()) then null
...
grant execute on function public.seller_has_active_paid_contact_access(uuid, timestamptz) to authenticated, service_role;
