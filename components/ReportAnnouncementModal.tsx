      ...
      <div className="w-full max-w-2xl rounded-[2rem] bg-white p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-100 pb-4">
          <div>
            ...
            <h3 className="mt-3 text-2xl font-black text-slate-900">Ajude a equipe a revisar este anúncio</h3>
        ...
        <div className="mt-6 space-y-3">
          {REPORT_REASON_OPTIONS.map((option) => {
            ...
            return (
              ...
              >
                <p className={`text-sm font-bold ${isSelected ? 'text-rose-800' : 'text-slate-900'}`}>{option.label}</p>
                <p className={`mt-1 text-sm leading-6 ${isSelected ? 'text-rose-700' : 'text-slate-500'}`}>{option.description}</p>
              </button>
            );
          })}
        </div>
        ...
        <div className="mt-6">
          ...
          <textarea
            ...
            className="min-h-[130px] w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm leading-6 text-slate-700 outline-none transition focus:border-rose-400 focus:ring-2 focus:ring-rose-100"
          />
        </div>

        <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-sm leading-6 text-amber-800">
          Após 10 denúncias de usuários únicos, o anúncio vai automaticamente para análise da equipe.
        </div>
